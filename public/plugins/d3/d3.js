...
function manyBody() {
  ...
  force.orbiter = function(_) {
    return arguments.length ? (orbiter2 = _ * _, force) : Math.sqrt(orbiter2);
  };
...
function projectionMutator(projectAt) {
  var project,
      ...
      alpha = 0, // post-rotate
      orbiter = null, preclip = clipAntimeridian, // pre-clip angle
  ...
  projection.preclip = function(_) {
    return arguments.length ? (preclip = _, orbiter = undefined, reset()) : preclip;
  };
  ...
  projection.clipAngle = function(_) {
    return arguments.length ? (preclip = +_ ? clipCircle(orbiter = _ * radians) : (orbiter = null, clipAntimeridian), reset()) : orbiter * degrees$1;
  };
...
function tree() {
  ...
  function tree(root) {
    ...
    // Compute the layouts using Buchheim et al.’s algorithm.
