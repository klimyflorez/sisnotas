...
/***/ "./resources/js/templates/admin/plugins/bootstrap-editable/bootstrap-editable.js":
/*!***************************************************************************************!*\
  !*** ./resources/js/templates/admin/plugins/bootstrap-editable/bootstrap-editable.js ***!
  \***************************************************************************************/
...
(function ($) {
  ...
  EditableForm.prototype = {
    ...
    initInput: function initInput() {
      ...
      this.input = this.options.input; //set initial value
      //todo: may be add check: typeof str === 'string' ?

      this.value = this.input.str2value(this.options.value); //prerender: get input.$input
    ...
    **/
    render: function render() {
      ...
      this.$div.empty().append(this.$loading); //init form template and buttons
      ...
      this.showLoading(); //flag showing is form now saving value to server.
      ...
      **/
      ...
      this.$div.triggerHandler('rendering'); //init input

      this.initInput(); //append input to form

      this.$form.find('div.editable-input').append(this.input.$tpl); //append form to container

      this.$div.append(this.$form); //render input
      ...
      $.when(this.input.render()).then($.proxy(function () {
        ...
        if (this.input.error) {
          ...
          this.input.$input.attr('disabled', true); //prevent form from submitting
          ...
          this.input.value2input(value); //attach submit handler
        ...
        **/
        ...
        this.showForm(); //call postrender method to perform actions required visibility of form
    ...
    submit: function submit(e) {
      e.stopPropagation();
      e.preventDefault(); //get new value from input

      var newValue = this.input.input2value(); //validation: if validate returns string or truthy value - means error
      ...
      this.isSaving = true; //sending data to server

      $.when(this.save(submitValue)).done($.proxy(function (response) {
        this.isSaving = false; //run success callback

        var res = typeof this.options.success === 'function' ? this.options.success.call(this.options.scope, response, newValue) : null; //if success callback returns false --> keep form open and do not activate input
        ...
        if (res && _typeof(res) === 'object' && res.hasOwnProperty('newValue')) {
    ...
    save: function save(submitValue) {
      ...
      */
      ...
      if (send) {
        //send to server
        this.showLoading(); //standard params
  ...
  */
  ...
  $.fn.editableform.Constructor = EditableForm; //defaults
  ...
  */
  ...
  $.fn.editableform.template = '<form class="form-inline editableform">' + '<div class="control-group">' + '<div><div class="editable-input"></div><div class="editable-buttons"></div></div>' + '<div class="editable-error-block"></div>' + '</div>' + '</form>'; //loading div

  $.fn.editableform.loading = '<div class="editableform-loading"></div>'; //buttons

  $.fn.editableform.buttons = '<button type="submit" class="editable-submit">ok</button>' + '<button type="button" class="editable-cancel">cancel</button>'; //error class attached to control-group

  $.fn.editableform.errorGroupClass = null; //error class attached to editable-error-block

  $.fn.editableform.errorBlockClass = 'editable-error'; //engine
...
(function ($) {
  ...
  $.fn.editableutils = {
    ...
    */
    sliceObj: function sliceObj(obj, keys, caseSensitive
    /* default: false */
    ) {
      var key,
          keyLower,
    ...
    */
    getConfigData: function getConfigData($element) {
      ...
      $.each($element.data(), function (k, v) {
        if (_typeof(v) !== 'object' || v && _typeof(v) === 'object' && (v.constructor === Object || v.constructor === Array)) {
          data[k] = v;
        }
      });
      return data;
        ...
        if (o.children) {
          ...
          if (isValArray) {
            if ($.grep(value, function (v) {
              return v == (o && _typeof(o) === 'object' ? valueProp(o) : o);
            }).length) {
              result.push(o);
            }
          } else {
            var itemValue = o && _typeof(o) === 'object' ? valueProp(o) : o;
    ...
    */
    createInput: function createInput(options) {
      var TypeConstructor,
          typeOptions,
          input,
          type = options.type; //`date` is some kind of virtual type that is transformed to one of exact types
...
(function ($) {
  ...
  Popup.prototype = {
    ...
    init: function init(element, options) {
      this.$element = $(element); //since 1.4.1 container do not use data-* directly as they already merged into options.

      this.options = $.extend({}, $.fn.editableContainer.defaults, options);
      this.splitOptions(); //set scope of form callbacks to element

      this.formOptions.scope = this.$element[0];
      this.initContainer(); //flag to hide container, when saving value will finish

      this.delayedHide = false; //bind 'destroyed' listener to destroy container when element is removed from dom

      this.$element.on('destroyed', $.proxy(function () {
        this.destroy();
      }, this)); //attach document handler to close containers on click / escape

      if (!$(document).data('editable-handlers-attached')) {
        //close all on escape
        $(document).on('keyup.editable', function (e) {
          if (e.which === 27) {
            $('.editable-open').editableContainer('hide'); //todo: return focus on element
          }
        ...
        $(document).on('click.editable', function (e) {
          ...
          '.modal-backdrop', '.bootstrap-wysihtml5-insert-image-modal', '.bootstrap-wysihtml5-insert-link-modal']; //check if element is detached. It occurs when clicking in bootstrap datepicker
    ...
    **/
    ...
    show: function show(closeAll) {
      ...
      */
      ...
      if (this.$form) {//todo: destroy prev data!
        //this.$form.destroy();
      }

      this.$form = $('<div>'); //insert form into container body
    ...
    **/
    ...
    @method setPosition()
    */
    setPosition: function setPosition() {//tbd in child class
    },
    save: function save(e, params) {
      ...
      **/
      this.$element.triggerHandler('save', params); //hide must be after trigger, as saving value may require methods of plugin, applied to input
  ...
  **/
  ...
  $.fn.editableContainer.Inline = Inline; //defaults
...
(function ($) {
  "use strict"; //copy prototype from EditableContainer
  //extend methods
  ...
  $.extend($.fn.editableContainer.Inline.prototype, $.fn.editableContainer.Popup.prototype, {
    ...
    initContainer: function initContainer() {
      ...
      this.$tip = $('<span></span>'); //convert anim to miliseconds (int)
...
(function ($) {
  ...
  var Editable = function Editable(element, options) {
    this.$element = $(element); //data-* has more priority over js options: because dynamically created elements may change data-*
  ...
  Editable.prototype = {
    ...
    init: function init() {
      ...
      this.options.name = this.options.name || this.$element.attr('id'); //create input of specified type. Input needed already here to convert value for initial display (e.g. show text by id for select)
      ...
      this.$element.addClass('editable'); //specifically for "textarea" add class .editable-pre-wrapped to keep linebreaks
    ...
    */
    initLive: function initLive() {
      //store selector
      var selector = this.options.selector; //modify options for child elements

      this.options.selector = false;
      this.options.autotext = 'never'; //listen toggle events
    ...
    */
    render: function render(response) {
      ...
      if (this.input.value2htmlFinal) {
        return this.input.value2html(this.value, this.$element[0], this.options.display, response); //if display method defined --> use it
      } else if (typeof this.options.display === 'function') {
        return this.options.display.call(this.$element[0], this.value, response); //else use input's original value2html() method
      } else {
    ...
    **/
    disable: function disable() {
      ...
      this.handleEmpty(this.isEmpty); //do not stop focus on this element
    ...
    **/
    option: function option(key, value) {
      ...
      this.options[key] = value; //disabled
    ...
    **/
    show: function show(closeAll) {
      ...
      if (!this.container) {
        ...
        this.$element.editableContainer(containerOptions); //listen `save` event
  ...
  **/
  ...
  $.fn.editable = function (option) {
      ...
      **/
      ...
      case 'submit':
        ...
        var config = arguments[1] || {},
            $elems = this,
            errors = this.editable('validate'); // validation ok

        if ($.isEmptyObject(errors)) {
          var ajaxOptions = {}; // for single element use url, success etc from options

          if ($elems.length === 1) {
            var editable = $elems.data('editable'); //standard params
            ...
            config.error = config.error || editable.options.error; // multiple elements
          } else {
          ...
          } : $.noop; // ajax error callabck

          ajaxOptions.error = typeof config.error === 'function' ? function () {
            config.error.apply($elems, arguments);
          } : $.noop; // extend ajaxOptions
    ...
    return this.each(function () {
      var $this = $(this),
          data = $this.data(datakey),
          options = _typeof(option) === 'object' && option; //for delegated targets do not store `editable` object for element
  ...
  $.extend(List.prototype, {
    ...
    onSourceReady: function onSourceReady(success, error) {
      ...
      if ($.isFunction(this.options.source)) {
        ...
        this.sourceData = null; //note: if function returns the same source as URL - sourceData will be taken from cahce and no extra request performed
      } else {
      ...
      if (typeof source === 'string') {
        ...
        if (this.options.sourceCache) {
          ...
          cache = $(document).data(cacheID); //check for cached data
          ...
          if (cache.loading === false && cache.sourceData) {
            ...
            cache.callbacks.push($.proxy(function () {
              ...
              this.doPrepend();
              success.call(this);
            }, this)); //also collecting error callbacks
        ...
        var ajaxOptions = $.extend({
          ...
          success: $.proxy(function (data) {
            ...
            if ($.isArray(this.sourceData)) {
              if (cache) {
                ...
                cache.sourceData = this.sourceData; //run success callbacks for other fields waiting for this source
          ...
          error: $.proxy(function () {
            ...
            if (cache) {
              cache.loading = false; //run error callbacks for other fields
        ...
        }, this.options.sourceOptions); //loading sourceData from server
    ...
    doPrepend: function doPrepend() {
      ...
      if (!$.isArray(this.prependData)) {
        ...
        this.options.prepend = $.fn.editableutils.tryParseJson(this.options.prepend, true); //convert prepend from string to object
    ...
    /*
     renders input list
    */
    renderList: function renderList() {// this method should be overwritten in child class
    },

    /*
    set element's html by value
    */
    value2htmlFinal: function value2htmlFinal(value, element) {// this method should be overwritten in child class
    },
        ...
        */
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          if (_typeof(item) === 'object') {
            ...
            $.each(item, iterateItem); //case: [{val1: 'text1'}, {val2: 'text2} ...]

            if (count === 1) {
              result.push(obj); //case: [{value: 1, text: 'text1'}, {value: 2, text: 'text2'}, ...]
            } else if (count > 1) {
    ...
    value2input: function value2input(value) {
      ...
      if ($.isArray(value) && value.length) {
        this.$input.each(function (i, el) {
          var $el = $(el); // cannot use $.inArray as it performs strict comparison
...
(function ($) {
  ...
  var Constructor = function Constructor(options) {
    ...
    this.sourceData = null; //placeholder
    ...
    if (!options.select2.tags && options.source) {
      var source = options.source; //if source is function, call it (once!)
      ...
      if (typeof source === 'string') {
        options.select2.ajax = options.select2.ajax || {}; //some default ajax params
    ...
    this.options.select2 = $.extend({}, Constructor.defaults.select2, options.select2); //detect whether it is multi-valued

    this.isMultiple = this.options.select2.tags || this.options.select2.multiple;
    this.isRemote = 'ajax' in this.options.select2; //store function returning ID of item
  ...
  $.extend(Constructor.prototype, {
    render: function render() {
      this.setClass(); //can not apply select2 here as it calls initSelection
    ...
    value2html: function value2html(value, element) {
      ...
      if (this.options.select2.tags) {
        ...
        data = value; //data = $.fn.editableutils.itemsByValue(value, this.options.select2.tags, this.idFunc);
      } else if (this.sourceData) {
        data = $.fn.editableutils.itemsByValue(value, this.sourceData, this.idFunc);
      } else {//can not get list of possible values
      ...
      if ($.isArray(data)) {
        ...
        text = [];
        $.each(data, function (k, v) {
          text.push(v && _typeof(v) === 'object' ? that.formatSelection(v) : v);
        });
      } else if (data) {
        text = that.formatSelection(data);
      }

      text = $.isArray(text) ? text.join(this.options.viewseparator) : text; //$(element).text(text);
    ...
    value2input: function value2input(value) {
      ...
      if (!this.$input.data('select2')) {
        ...
        this.$input.val(value).trigger('change', true); //Uncaught Error: cannot call val() if initSelection() is not defined
...
(function ($) {
  ...
  Combodate.prototype = {
    ...
    init: function init() {
      ...
      this.initCombos(); //update original input on change

      this.$widget.on('change', 'select', $.proxy(function (e) {
        this.$element.val(this.getValue()).change(); // update days count if month or year changes
      ...
      this.$widget.find('select').css('width', 'auto'); // hide original input and insert widget

      this.$element.hide().after(this.$widget); // set initial value
    ...
    */
    getTemplate: function getTemplate() {
      var tpl = this.options.template; //first pass
    ...
    */
    initCombos: function initCombos() {
      for (var k in this.map) {
        var $c = this.$widget.find('.' + k); // set properties like this.$day, this.$month etc.

        this['$' + k] = $c.length ? $c : null; // fill with items
    ...
    */
    fillCommon: function fillCommon(key) {
      ...
      if (this.options.firstItem === 'name') {
        ...
        var header = typeof relTime[key] === 'function' ? relTime[key](1, true, key, false) : relTime[key]; //take last entry (see momentjs lang files structure)
    ...
    */
    fillDay: function fillDay() {
      var items = this.fillCommon('d'),
          ...
          daysCount = 31; // detect days count (depends on month and year)
    ...
    */
    getValue: function getValue(format) {
      var dt,
          values = {},
          that = this,
          notSelected = false; //getting selected values
      ...
      dt = moment([values.year, values.month, values.day, values.hour, values.minute, values.second]); //highlight invalid date
    ...
    setValue: function setValue(value) {
      ...
      var dt = typeof value === 'string' ? moment(value, this.options.format) : moment(value),
          that = this,
          values = {}; //function to find nearest value in select options
  ...
  $.fn.combodate = function (option) {
    ...
    args.shift(); //getValue returns date as string / object (not jQuery object)
...
(function ($) {
  ...
  var Constructor = function Constructor(options) {
    this.init('combodate', options, Constructor.defaults); //by default viewformat equals to format
    ...
    options.combodate = $.fn.editableutils.tryParseJson(options.combodate, true); //overriding combodate config (as by default jQuery extend() is not recursive)
...
(function ($) {
  ...
  $.extend($.fn.editableform.Constructor.prototype, {
    ...
    initInput: function initInput() {
      pInitInput.apply(this); //for bs3 set default class `input-sm` to standard inputs

      var emptyInputClass = this.input.options.inputclass === null || this.input.options.inputclass === false;
      var defaultClass = 'input-sm'; //bs3 add `form-control` class to standard inputs
  ...
  $.fn.editableform.buttons = '<button type="submit" class="btn btn-info btn-sm editable-submit">' + '<i class="fa fa-check"></i>' + '</button>' + '<button type="button" class="btn btn-inverse btn-sm editable-cancel">' + '<i class="fa fa-close"></i>' + '</button>'; //error classes
...
(function ($) {
  ...
  $.extend(DateField.prototype, {
    render: function render() {
      ...
      this.$tpl.bdatepicker(this.options.datepicker); //need to disable original event handlers
...
/***/ 1:
/*!*********************************************************************************************!*\
  !*** multi ./resources/js/templates/admin/plugins/bootstrap-editable/bootstrap-editable.js ***!
  \*********************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! C:\laragon\www\sisnotas\resources\js\templates\admin\plugins\bootstrap-editable\bootstrap-editable.js */"./resources/js/templates/admin/plugins/bootstrap-editable/bootstrap-editable.js");
