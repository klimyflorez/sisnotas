...
;(function($, window, document, undefined) {
	...
	DatePaginator.prototype = {
		...
		_back: function() {
			this._setSelectedDate(this.options.selectedDate.clone().subtract(1, 'day'));
			this._render();
		},

		_forward: function() {
			this._setSelectedDate(this.options.selectedDate.clone().add(1, 'day'));
			this._render();
	    },
		...
		_buildData: function() {
			...
			var viewWidth = (this.options.width - ((this.options.selectedItemWidth - this.options.itemWidth) + (this.options.navItemWidth * 2))),
				...
				start = this.options.selectedDate.clone().subtract(unitsPerSide, 'days'),
				end = this.options.selectedDate.clone().add((units - unitsPerSide), 'days');
